...
let rec fltr attr a1 a2 cmp row = 
    ...
    let _ = Printf.printf "Comparing values %s and %s\n" a a' in    
    let resu = cmp a a' in 
    let _ = Printf.printf "The result is : %s\n\n" (string_of_bool resu) in resu
...
and fltr_rw attr c row = match c with
    ...
    | In (Attrs a, op) -> let table = eval op in 
...
and eval : (Algebra.t -> Data.table) = fun op -> 
  ...
  | Project (r, proj) ->
      ...
      let _ = Printf.printf "Evaluating index of projection %s\n" (List.fold_left (fun a b -> a^(show_attr_bind b)) "" proj) in
      let _ = Printf.printf "Index to be kept are:\n%s\n\n" (show_index slct) in
      let _ = Printf.printf "On attributes %s\n\n" (List.fold_left (fun a b -> a^(show_attr_bind b)) "" r'.attr) in
