...
query:
  ...
  | SELECT p=proj r=from ORDER a=attrs DESC WHERE c=cond { Order (a, Select (p, r, Some c), true) } 
  | SELECT p=proj r=from ORDER a=attrs WHERE c=cond      { Order (a, Select (p, r, Some c), false) } 
  | SELECT p=proj r=from ORDER a=attrs                   { Order (a, Select (p, r, None), false) }
  | SELECT p=proj r=from ORDER a=attrs DESC              { Order (a, Select (p, r, None), true) }
